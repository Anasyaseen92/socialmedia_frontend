...
const MyPostWidget = ({ picturePath }) => {
  ...
  const [post, setPost] = useState(""); // this is description
  const { palette } = useTheme();
  const user = useSelector((state) => state.user); // ✅ fix
  ...
  const handlePost = async () => {
    ...
    const response = await fetch("https://socialmedia-backend-9nw8.vercel.app/posts", {
      ...
      body: JSON.stringify({
        userId: user._id,
        description: post, // ✅ FIXED: this is your post text
        picturePath: publicUrl, // ✅ this is image URL or empty string
      }),
    });
